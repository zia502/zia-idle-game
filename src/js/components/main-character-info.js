class MainCharacterInfo {
    ...
    render() {
        ...
        if (inDungeon) {
            console.log('角色在地下城中，使用currentStats作为基础');
            totalAttack = character.currentStats?.attack || 0;
            totalHp = character.currentStats?.hp || 0;
        } else {
            // 不在地下城中，尝试使用 Character.getCharacterFullStats 获取完整属性
            try {
